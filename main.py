...
def maxProfit(k: int, prices: List[int]) -> int:
    ...
    for i in range(len(simple_list), 0, -2):
        for j in range(i - 1, 0, -2):
            look_up[j-1, i-1] = k_1_max(simple_list[j-1: i])

    # print(sorted(look_up.items(), key=lambda x: x[1], reverse=True))
    print(look_up[(0, 1)])
    simple_indices = list(range(len(simple_list)))
    # print(simple_indices)
    print(recursion_k(simple_indices, k, look_up))


def recursion_k(recursive_list, k_, look_up):
    max_ = 0
    if recursive_list == [] or k_ <= 0:
        return 0

    for i in range(len(recursive_list))[::2]:
        # print("i =", i)
        high = recursive_list[i + 1]
        low = recursive_list[0]
        # print("low high:", low, high)
        temp_ = look_up[(low, high)] + recursion_k(recursive_list[i + 2:], k_ - 1, look_up)
        max_ = max(max_, temp_)

    return max_


def k_1_max(price_list: List[int]) -> int:

    profit = 0
    big = 0

    for day in reversed(price_list):
        if big == 0:
            big = day
            continue
        profit = max(profit, big-day)
        big = max(big, day)
    return profit
